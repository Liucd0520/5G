...
class DoubleTower(nn.Module):
    def __init__(self,
        ...
        self.model_dce = monai.networks.nets.resnet50(spatial_dims=3, n_input_channels=1, num_classes=2, feed_forward=False).to(self.device)
        self.model_adc = monai.networks.nets.resnet50(spatial_dims=3, n_input_channels=1, num_classes=2, feed_forward=False).to(self.device)
        ...
        self.Linear1 = Linear(2048, self.fc_hidden_size, device=self.device)  # 1024 是 所有下采样特征图globalpool之后拼接的结果
...
dce_pretrain_path = adc_pretrain_path = '/app/liucd/deeplearn_dec/DL_dec/pretrain/resnet_50.pth'
#dce_pretrain_path = ''
#adc_pretrain_path = ''
