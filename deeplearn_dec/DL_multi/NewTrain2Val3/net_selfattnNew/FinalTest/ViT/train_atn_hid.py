...
class DoubleTower(nn.Module):
    def __init__(self,
        ...
        self.vit = monai.networks.nets.ViT(in_channels=1, img_size=(96, 96, 32), 
                              hidden_size=128, num_layers=6, num_heads=8, 
                              patch_size=(16, 16, 16), pos_embed='conv', classification=False).to(self.device)

        self.Linear1 = Linear(128, self.fc_hidden_size, device=self.device)  # 1024 是 所有下采样特征图globalpool之后拼接的结果
    ...
    def forward(self, x1, x2, rad, structured_data):  # x 是SegResNet的输入影像矩阵
        ...
        encode_output1, _ = self.vit(x1)
        fc1 = F.relu(self.Linear1(encode_output1.sum(dim=1)))
...
for epoch in range(max_epochs):
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        with torch.no_grad():
            ...
            if tot_metric > best_metric and trainval_auc_result > 0.87 and auc_result > 0.85:
