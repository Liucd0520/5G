...
class DoubleTower(nn.Module):
    def __init__(self,
        ...
        self.Linear1 = Linear(512 + 9, self.fc_hidden_size , device=self.device)  # 1024 是 所有下采样特征图globalpool之后拼接的结果
    ...
    def forward(self, x1, x2, rad, structured_data):  # x 是SegResNet的输入影像矩阵
        ...
        #print(encode_output1.shape, rad.shape)

        fc1 = F.relu(self.Linear1(torch.concat([encode_output1, rad], dim=-1)))
