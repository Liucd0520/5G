...
syf_adcdir = '/app/liucd/deeplearn_dec/DL_dec/data_adc/syf/NonPCR'
zy_adcdir = '/app/liucd/deeplearn_dec/DL_dec/data_adc/zunyi/NonPCR'
sd_adcdir = '/app/liucd/deeplearn_dec/DL_dec/data_adc/shandong/NonPCR'
yizhong_adcdir = '/app/liucd/deeplearn_dec/DL_dec/data_adc/yizhong/NonPCR'
xian_adcdir = '/app/liucd/deeplearn_dec/DL_dec/data_adc/xian/NonPCR'

syf_dcedir = '/app/liucd/deeplearn_dec/DL_dec/data/syf/NonPCR'
zy_dcedir = '/app/liucd/deeplearn_dec/DL_dec/data/zunyi/NonPCR'
sd_dcedir = '/app/liucd/deeplearn_dec/DL_dec/data/shandong/NonPCR'
yizhong_dcedir = '/app/liucd/deeplearn_dec/DL_dec/data/yizhong/NonPCR'
xian_dcedir = '/app/liucd/deeplearn_dec/DL_dec/data/xian/NonPCR'
...
for file_path in train_adcimages:
    ...
    train_label.append(int(df_raw[df_raw['patient_ID'] == int(p_id)].values.tolist()[0][-1]))
...
for file_path in val_adcimages:
    ...
    val_label.append(int(df_raw[df_raw['patient_ID'] == int(p_id)].values.tolist()[0][-1]))

    
train_dict = [{'image_adc': image_adc, 'image_dce': image_dce, 'rad':rad, 'clinical': clinical,  'label': lab}
                  for image_adc, image_dce, rad, clinical, lab in zip(train_adcimages,  train_dceimages, train_rad, train_clinical, train_label)]
val_dict = [{'image_adc': image_adc, 'image_dce': image_dce,'rad': rad, 'clinical': clinical,  'label': lab}
                  for image_adc, image_dce,rad, clinical,lab in zip(val_adcimages, val_dceimages,val_rad, val_clinical, val_label)]
...
for epoch in range(max_epochs):
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        with torch.no_grad():
            ...
            if tot_metric > best_metric and trainval_auc_result > 0.92 and auc_result > 0.856:
