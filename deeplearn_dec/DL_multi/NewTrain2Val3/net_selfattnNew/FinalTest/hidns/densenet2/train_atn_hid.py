...
train_transforms = Compose(
        [
            ...
            Resized(keys=["image_adc"], spatial_size=(64, 64, 32)),
...
class DoubleTower(nn.Module):
    def __init__(self,
        ...
        self.model_adc = monai.networks.nets.DenseNet(3, 1, 2).to(self.device)
        ...
        self.attn = nn.MultiheadAttention(1024, num_heads=8, batch_first=True, device=self.device)
    ...
    def forward(self, x1, x2, rad, structured_data):  # x 是SegResNet的输入影像矩阵
        ...
        _, encode_output1 = self.model_dce(x1)  # 修改了densenet网络
        _, encode_output2 = self.model_adc(x2)

        encode_output1 = torch.nn.functional.adaptive_avg_pool3d(encode_output1, (1, 1, 1)).squeeze()
        encode_output2 = torch.nn.functional.adaptive_avg_pool3d(encode_output2, (1, 1, 1)).squeeze()
