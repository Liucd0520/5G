...
syf_adcpcrdir = '/app/liucd/deeplearn_dec/DL_dec/data_adc/syf/PCR'
zy_adcprcdir = '/app/liucd/deeplearn_dec/DL_dec/data_adc/zunyi/PCR'
sd_adcpcrdir = '/app/liucd/deeplearn_dec/DL_dec/data_adc/shandong/PCR'
yizhong_adcpcrdir = '/app/liucd/deeplearn_dec/DL_dec/data_adc/yizhong/PCR'
xian_adcpcrdir = '/app/liucd/deeplearn_dec/DL_dec/data_adc/xian/PCR'
...
syf_dcepcrdir = '/app/liucd/deeplearn_dec/DL_dec/data/syf/PCR'
zy_dcepcrdir = '/app/liucd/deeplearn_dec/DL_dec/data/zunyi/PCR'
sd_dcepcrdir = '/app/liucd/deeplearn_dec/DL_dec/data/shandong/PCR'
yizhong_dcepcrdir = '/app/liucd/deeplearn_dec/DL_dec/data/yizhong/PCR'
xian_dcepcrdir = '/app/liucd/deeplearn_dec/DL_dec/data/xian/PCR'


train_adcimages = sorted(glob.glob(os.path.join(syf_adcdir,  '*.nii.gz'))) + \
                  sorted(glob.glob(os.path.join(zy_adcdir,  '*.nii.gz'))) 

train_adcpcrimages = sorted(glob.glob(os.path.join(syf_adcpcrdir,  '*.nii.gz'))) + \
                    sorted(glob.glob(os.path.join(zy_adcprcdir,  '*.nii.gz'))) + \
                    sorted(glob.glob(os.path.join(sd_adcpcrdir,  '*.nii.gz'))) + \
                    sorted(glob.glob(os.path.join(yizhong_adcpcrdir,  '*.nii.gz'))) + \
                    sorted(glob.glob(os.path.join(xian_adcpcrdir,  '*.nii.gz'))) 


train_dceimages = sorted(glob.glob(os.path.join(syf_dcedir,  '*.nii.gz'))) + \
                 sorted(glob.glob(os.path.join(zy_dcedir,  '*.nii.gz'))) 

train_dcepcrimages =  sorted(glob.glob(os.path.join(syf_dcepcrdir,  '*.nii.gz'))) + \
                    sorted(glob.glob(os.path.join(zy_dcepcrdir,  '*.nii.gz'))) + \
                    sorted(glob.glob(os.path.join(sd_dcepcrdir,  '*.nii.gz'))) + \
                    sorted(glob.glob(os.path.join(yizhong_dcepcrdir,  '*.nii.gz'))) + \
                    sorted(glob.glob(os.path.join(xian_dcepcrdir,  '*.nii.gz'))) 
...
print(len(val_adcimages), len(val_dceimages))
train_clinical = []
...
def gen_clinical():
    import random
    t_stage = random.randint(1, 3)
    her2 = random.randint(0, 2)
    nac = random.randint(1, 3)
    er = random.random()
    pr = random.random()
    ki67 = random.random()
    return t_stage, her2, nac, er, pr, ki67

train_pcrclinical = []
for file_path in train_adcpcrimages:
    train_pcrclinical.append(gen_clinical())
...
train_dict = [{'image_adc': image_adc, 'image_dce': image_dce, 'clinical': clinical,  'label': int(image_adc.split('_')[-1].replace('.nii.gz', ''))}  for image_adc, image_dce, clinical in zip(train_adcimages,  train_dceimages, train_clinical)]

train_pcrdict = [{'image_adc': image_adc, 'image_dce': image_dce, 'clinical': clinical,  'label': int(image_adc.split('_')[-1].replace('.nii.gz', ''))}   for image_adc, image_dce, clinical in zip(train_adcpcrimages,  train_dcepcrimages, train_pcrclinical)]

train_dict = train_dict + train_pcrdict
