...
train_ds = Dataset(data=train_dict, transform=train_transforms,)# cache_rate=1.0, num_workers=12)
val_ds = Dataset(data=val_dict, transform=val_transforms,)# cache_rate=1.0, num_workers=12)
...
class DoubleTower(nn.Module):
    def __init__(self,
        ...
        self.model_dce = monai.networks.nets.DenseNet(spatial_dims=3, in_channels=1, out_channels=2).to(self.device)
        self.model_adc = monai.networks.nets.DenseNet(spatial_dims=3, in_channels=1, out_channels=2).to(self.device)
    ...
    def forward(self, x1, x2, structured_data):  # x 是SegResNet的输入影像矩阵
        ...
        return F.log_softmax(encode_output, dim=-1)
