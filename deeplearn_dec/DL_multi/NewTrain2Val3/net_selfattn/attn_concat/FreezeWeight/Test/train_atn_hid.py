...
trainval_loader = DataLoader(trainval_ds, batch_size=8, num_workers=8, pin_memory=True)
...
dce_dict.update(dce_pretrain_dict)
model.load_state_dict(dce_dict)

for name, param in model.named_parameters():
    if name != 'Linear2.weight' and name != 'Linear2.bias' and name != 'Linear1.weight' and name != 'Linear1.bias' :
...
for epoch in range(max_epochs):
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        with torch.no_grad():
            ...
            if tot_metric > best_metric and trainval_auc_result > 0.84 and auc_result > 0.79:
