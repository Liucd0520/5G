...
data_dir = os.path.normpath('/app/liucd/Crop')

all_images = sorted(glob.glob(os.path.join(data_dir, 'crop_image', '*.nii.gz')))
all_labels = sorted(glob.glob(os.path.join(data_dir, 'crop_label', '*.nii.gz')))
...
train_transforms = Compose(
        [
            ...
            SpatialPadd(keys=["image", "label"], spatial_size=(224, 224, 224)),
            RandCropByPosNegLabeld(
                keys=["image", "label"],
                label_key="label",
...
optimizer = torch.optim.Adam(model.parameters(), 1e-4, weight_decay=1e-6)
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        with torch.no_grad():
            for val_data in val_loader:
                ...
                sw_batch_size = 1  # 这里的sw_batch_size 应该是slide总数以batch为4的小图进行推理，而不是取验证集里的4张大图进行推理
